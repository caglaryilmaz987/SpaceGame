...
public class HabitablePlanet extends Planet {
    ...
    public HabitablePlanet(String name, String atmosphere, int gravity, Metal metal, Uranium uranium, RepconnSpaceRocket rocket) {
        super(name, atmosphere, gravity, metal, uranium, rocket);
    }

    // I didn't want to formula it, I thought specific ranges would make more sense
    @Override
    public int landingCost(RepconnSpaceRocket rocket) {
        // We need to keep track of the initial fuel level
        int initialFuel = rocket.getFuelStatus();

        if (gravity < 5 && gravity >= 0) {
            rocket.setFuelStatus(rocket.getFuelStatus());
            if (atmosphere.equals(atmosphere)) {
                rocket.setFuelStatus(rocket.getFuelStatus());
            }
        } else if (gravity < 10 && gravity >= 5) {
            rocket.setFuelStatus(rocket.getFuelStatus() - 1);
            if (atmosphere.equals(atmosphere)) {
                rocket.setFuelStatus(rocket.getFuelStatus());
            }
    ...
    // I want to make resource extraction different for each planet, that's why I'm overriding the general abstract method
    @Override
    public void collectResources(RepconnSpaceRocket rocket) {
        int totalResources = getMetal().getQuantity() + getUranium().getQuantity();
        if (rocket.getFuelStatus() < 3 && rocket.getHellFire().quantity <= 0 && rocket.getX_01().quantity <= 0 && rocket.getX_02().quantity <= 0) {
            ...
            System.out.println("\n");
            System.out.println("If you want choose " + rocket.getHellFire().getName() + " model please sign 1");
            System.out.println("\n");
            System.out.println("If you want choose " + rocket.getX_01().getName() + " model please sign 2");
            System.out.println("\n");
            System.out.println("If you want choose " + rocket.getX_02().getName() + " model please sign 3");
            System.out.println("\n");
            ...
            switch (pow_arm_choice) {
                case 1:
                    ...
                    // I formula-ed the resource extraction process
                    rocket.setFuelStatus(rocket.getFuelStatus() - ((int) (totalResources * 0.1) + 1));
                    ...
                    // Resources should be loaded into the rocket as cargo
                ...
                case 2:
                    ...
                    rocket.getUranium().increaseQuantity(getUranium().getQuantity());
                    // I formula-ed the resource extraction process
                    rocket.setFuelStatus(rocket.getFuelStatus() - ((int) (totalResources * 0.1) - 3));
                    System.out.println("Commander, the enclave soldiers have successfully completed the resource extraction mission.");
                    System.out.println("");
                    ...
                    // Resources should be loaded into the rocket as cargo
                    rocket.setCargo(rocket.getCargo() + totalResources);
                    int newQuantity2 = rocket.getX_01().getQuantity() - 2;
                    rocket.setHellFire(new HellFire(newQuantity2));
                ...
                case 3:
                    ...
                    rocket.getUranium().increaseQuantity(getUranium().getQuantity());
                    // I formula-ed the resource extraction process
                    rocket.setFuelStatus(rocket.getFuelStatus() - ((int) (totalResources * 0.1) - 5));
                    System.out.println("Commander, the enclave soldiers have successfully completed the resource extraction mission.");
                    System.out.println("");
                    ...
                    // Resources should be loaded into the rocket as cargo
                    rocket.setCargo(rocket.getCargo() + totalResources);
                    int newQuantity3 = rocket.getHellFire().getQuantity() - 1;
                    rocket.setHellFire(new HellFire(newQuantity3));
            ...
            switch (pow_arm_choice) {
                ...
                case 3:
                    ...
                    rocket.setFuelStatus(rocket.getFuelStatus() - ((int) (collectedAmount * 0.1) - 5));
    ...
    public String toString() {
        return "Commander, here is the information about the planet we successfully landed on. Long live the enclave." + " "
                + "Planet name: " + getName() + " " + "Planet atmosphere type: " + getAtmosphere() + " "
                + "Planet X coordinate: " + getX() + " " + "Planet Y coordinate: " + getY() + " "
                + "Planet gravity: " + getGravity() + " " + "Amount of metal resources on the planet: " + metal.getQuantity() + " "
